...
pluginInputs = [
  ...
  },
  {
    ...
    placeholder: "默认不施肥",
    type: "select",
    selects: [{ name: '不施肥', value: 'no' }, { name: '每阶段施肥', value: 'every' }, { name: '收获前施肥', value: 'last' }],
  },
  {
    key: "reload",
    title: "【农场种植】重载时间",
    placeholder: "默认1小时",
    type: "select",
    selects: [{ name: '15分钟', value: 900 }, { name: '30分钟', value: 1800 }, { name: '60分钟', value: 3600 }, { name: '120分钟', value: 7200 }],
  },
  {
    key: "sgncQuestion",
    title: "【时光农场】答题是否启用",
  ...
  do {
    try {
      ...
      if (!xiaotan_bug_report_finished) {
        ...
        let saleNumInput = Number(shark.rule.saleNum) || 60;
        saleNumInput = saleNumInput >= 60 ? 60 : saleNumInput;
        const cropId = splitStringId(shark.rule.crop || "747,268,304,72,78,76,33,98,38,37,228,77,51,308,305,34");
        // 获取小摊数据
        const { buyStrangerTimes, level } = await requestXiaoTanInfo(shark.selfUin, `【小摊广告小报】获取自己小摊数据`);
        await shark.delay(10)
        //根据当前等级获取用户的购买次数
        const { strangerTimes } = getBuyNumFn(level);
        if (buyStrangerTimes === strangerTimes) {
        ...
        await shark.delay(5)
        xiaotanDataFor: for (let i = 0; i < xiaotanData.length; i++) {
          const { saleMoneyType, saleItemNum, saleItemid, uin } = xiaotanData[i];
          if (saleMoneyTypeSelected.includes(saleMoneyType) && cropId.includes(saleItemid) && saleItemNum >= saleNumInput) {
            //进入小摊
            const friendXiaotanData = await requestXiaoTanInfo(uin, `【小摊广告小报】获取广告小报${uin}的数据`);
            await shark.delay(5);
            const needList = friendXiaotanData.shelf;
            for (let j = 0; j < needList.length; j++) {
              const { saleInfo, status } = needList[j];
              if (
                ...
                // 购买
                const { ecode, direction } = await requestXiaoTanByg(uin, j, `【小摊广告小报】开始购买广告小报${uin}的小摊作物`);
                await shark.delay(10)
                if (ecode === 0) {
                  shark.logger.log(`【小摊广告小报】广告小报购买成功：${getSeedInfoFn(saleInfo.saleItemid)?.name}*${saleInfo.saleItemNum}个`);
                  hasTimes--;
                  if (hasTimes === 0) {
                    shark.setCache("xiaotan_bug_report_finished", true, shark.getNextDayMS());
                    shark.logger.log("【小摊广告小报】广告小报购买次数已用完");
                  ...
                  // 当出现购买不成功时跳出循环
                  shark.logger.log('【小摊广告小报】' + direction)
                  break xiaotanDataFor
                }
              }
            }
      ...
      const nextDayTime = (shark.getNextDayMS() / 1000 - shark.getFarmTime()) + 30;
...
async function bugFriendXiaoTan() {
  do {
    ...
    try {
      const xiaotan_friends = splitStringId(shark.rule.xiaotan_friends);
      // 过滤掉自己的qq
      xiaotan_friends?.indexOf(shark.selfUin) > -1 ? xiaotan_friends.splice(xiaotan_friends?.indexOf(shark.selfUin), 1) : null;
      // 好友次数是否用完
      const xiaotan_bug_friend_finished = shark.getCache("xiaotan_bug_friend_finished");
      // 存放已经超过购买次数的好友qq
      const xiaotanFriendNoFriend = shark.getCache("xiaotanFriend_no_friend") || []; // 存放已经超过购买次数的好友qq
      if (xiaotan_friends.length > 0 && !xiaotan_bug_friend_finished && xiaotanFriendNoFriend.length < xiaotan_friends.length) {
        // 获取自己小摊数据
        const { buyFriendTimes, level } = await requestXiaoTanInfo(shark.selfUin, `【小摊好友】获取自己小摊数据`);
        await shark.delay(15);
        //根据当前等级获取用户的购买次数
        const { friendTimes } = getBuyNumFn(level);
        if (buyFriendTimes === friendTimes) {
          shark.setCache("xiaotan_bug_friend_finished", true, shark.getNextDayMS());
        ...
        let hasTimes = friendTimes - buyFriendTimes;
        xiaotanDataFor: for (let friendUin of xiaotan_friends) {
          const newXiaotanFriendNoFriend = shark.getCache("xiaotanFriend_no_friend") || [];
          if (newXiaotanFriendNoFriend.includes(friendUin)) {
            // 如果当前好友的交易次数已经达到限制，那么就遍历下一个好友
            continue;
          }
          //进入小摊
          const { shelf } = await requestXiaoTanInfo(friendUin, `【小摊好友】获取好友${friendUin}的小摊数据`);
          await shark.delay(10);
          for (let j = 0; j < shelf.length; j++) {
            const { saleInfo, status } = shelf[j];
            if (status === 1) {
              // 购买
              const { ecode, direction } = await requestXiaoTanByg(friendUin, j, `【小摊好友】开始购买好友${friendUin}第${j + 1}个摊位的作物`);
              await shark.delay(10);
              if (ecode === 0) {
                shark.logger.log(`【小摊好友】购买好友${friendUin}的作物：${getSeedInfoFn(saleInfo.saleItemid)?.name}*${saleInfo.saleItemNum}个`);
                hasTimes--;
                if (hasTimes === 0) {
                  // 当已经没有购买次数时
                  shark.setCache("xiaotan_bug_friend_finished", true, shark.getNextDayMS());
                ...
                if (direction?.indexOf("该好友生意太火爆了") > -1) {
                  if (!newXiaotanFriendNoFriend.includes(friendUin)) {
                    ...
                    shark.setCache("xiaotanFriend_no_friend", newXiaotanFriendNoFriend, shark.getNextDayMS());
                  }
                  break;
                } else {
                  shark.logger.log("【小摊好友购买】" + direction)
                  break xiaotanDataFor
                }
              }
              await shark.delay(10);
  ...
  try {
    ...
    if (!couponToThing) {
      for (let i = 0; i < couponToThingSelected.length; i++) {
        ...
        if (ecode === 0) {
          shark.logger.log(
            `【点券兑换】：花费点卷${coupon?.toString()?.substring(1)}兑换${couponToThingSelectedArr[2]
            }`
          );
...
async function sgncQuestionFn() {
  try {
    const { result: cacheResult = -1, options: cacheOptions = [], preAnswerTime = null } = shark.getCache('sgncQuestion') || {};
    let toDayIsAnswer = false;
    if (preAnswerTime && new Date(preAnswerTime * 1000).getDay() === new Date(shark.getFarmTime() * 1000).getDay()) {
      toDayIsAnswer = true
    }
    if (cacheResult === -1 && !toDayIsAnswer) {
      ...
      if (ecode === 0) {
        ...
        if (result === -1) {
          //fin===false代表今天未答题
          if (fin === false) {
            const todayOption = [1, 2, 3, 4].reduce((pre, cur) => { return pre || (cacheOptions.includes(cur) ? 0 : cur) }, 0)
            const { ecode: qustionCode, pkg, result: qustionResult } = await shark.reqest({
              name: '【时光农场】每日答题开始作答',
              url: "{!nc}exchange",
            ...
            if (qustionCode === 0) {
              if (qustionResult === 1) {
                shark.logger.log(`【时光农场】今日答题正确:${shark.convertPkgToStr(pkg)}`)
              } else {
                shark.logger.log(`【时光农场】今日答题错误(选择了选项${todayOption})，明天继续`)
              }
              cacheOptions.push(todayOption)
              shark.setCache('sgncQuestion', { result: qustionResult, options: cacheOptions, preAnswerTime: shark.getFarmTime() }, shark.getNextWeekDayMS(4));
            }
          } else {
            ...
            shark.setCache('sgncQuestion', { result: result, options: cacheOptions, preAnswerTime: shark.getFarmTime() }, shark.getNextWeekDayMS(4));
          }
        } else {
...
async function requestGetUserLand() {
  ...
  }, (data) => { return data?.farmlandStatus ? true : false });
...
async function requestPlanting(landNum, seedId, packageData) {
  ...
  if (Array.isArray(realSeedId)) {
    ...
    for (let i = 0; i < realSeedId.length; i++) {
      const { amount, thingObj } = getPackageThingNumFn(packageData, 'cId', 1, realSeedId[i])
      if (amount > 0) {
    ...
    if (Array.isArray(realSeedId)) {
      realSeedId = null;
      shark.logger.log(`【农场种植】第${landNum + 1}块土地种植失败：背包里已经没有节气种子了`)
    ...
    const { amount, thingObj } = getPackageThingNumFn(packageData, 'cId', 1, realSeedId)
    if (amount > 0) {
      packageSeedObj = thingObj;
    } else {
  ...
  if (code === 1) {
    ...
    shark.logger.log(`【农场种植】第${landNum + 1}块土地种植${seedName || ''}失败：${direction}`)
  }
...
async function requestHarvest(landNum) {
  ...
  if (code === 1) {
    shark.logger.log(`【农场种植】第${landNum + 1}块土地收获成功:${getSeedInfoFn(status?.cId)?.name}*${harvest}个`)
  } else {
    shark.logger.log(`【农场种植】第${landNum + 1}块土地收获失败：${direction}`)
  }
...
async function requestScarify(landNum, isForce = false) {
  ...
  if (code === 1) {
    ...
    shark.logger.log(`【农场种植】第${landNum + 1}块土地铲除失败：${direction}`)
  }
  return { code, direction }
}
// 施肥
async function requestFertilize(landNum) {
  ...
  if (code === 1) {
    ...
    shark.logger.log(`【农场种植】第${landNum + 1}块土地施肥失败：${direction}`)
  }
  return { code, direction, status }
}
// 获取背包数据
...
async function requestFriendDetail(type, qq, name, uid) {
  const responseData = await requestOptimize({
    name: `【农场种植】${type === 1 ? "获取" : "刷新"}好友[QQ:${qq},name:${name}]信息`,
    url: "{!nc}cgi_farm_index",
...
async function requestSneaking(qq, name, uid, landNum, seedId) {
  const { ecode, direction } = await shark.reqest({
    name: `【农场种植】偷取[qq:${qq},name:${name}]的土地${landNum}:${getSeedInfoFn(seedId)?.name}(id=${seedId})`,
...
function getSeedInputDataFn() {
  // eg: 1-20:12;21-30:99
  const seedInputDataInfo = {}, errorRule = []
  ...
  if (errorRule.length > 0) {
    shark.logger.error(`【农场种植】以下种子规则填写错误：${errorRule},请重新填写`)
  }
...
function forbidLandFn(landNum, time = 300) {
  ...
  if (forbidLand) {
    ...
    shark.setCache(`forbidLand${landNum}`, { startTime: nowFarmTime, countdown: timeRange }, timeOut)
  }
}

//通过解析接口返回值来判断当前种子是否可以施肥如果不可以则记录到cache中
function seedIsNotFertilizerFn(seedId, responseData = {}) {
  if (responseData.code === 0 && responseData.direction?.indexOf('不能对该作物使用') >-1) {
...
function isFertilizerFn(seedId) {
  ...
  if (isYouji !== 1 && !solarTerms.find(i => i.id === seedId) && !seedNotFertilizer.includes(seedId)) {
    return true
  } else {
    return false
  }
...
async function plantFertilizeFn(landNum, land, packageData) {
  ...
  const { a: seedId, b: stage, s: landType, q: plantTime, o: hasFertilizer } = land || {};
  if (seedId) {
    // 背包是有化肥
    ...
    if (!isLandFertilizerFn(land) && isFertilizerFn(seedId) && amount > 0) {
      ...
      if (fertilizer === 'last') {
        if (isFinalStageFn(seedId, landType, plantTime) && nextStageTime <= 3600) {
          //施肥
          ...
          if (responseData.code === 1) {
            ...
            await operationHarvestFn(landNum, land, packageData, responseData.status)
            await shark.delay(1)
...
function isFinalStageFn(seedId, landType, plantTime, time = shark.getFarmTime()) {
...
function nextStageTimeFn(seedId, landType, plantTime) {
  ...
  const nextStageTime = getSeedStepInfoFn(seedId, landType).reduce((prev, curr) => {
    if (prev > 0) {
      return prev
    }
...
function getPackageThingNumFn(packageData = [], idType, type, id) {
  const thingObj = packageData.find(i => i?.[idType] === id && i.type === type) || {}
...
function ArrayObjectToDeduplicationFn(list, key) {
  let temp = []
  return list.filter((item) => !temp.includes(item[key]) && temp.push(item[key]))
...
function secondsToHoursMinutesSecondsFn(num) {
  ...
  return { hours, minutes, seconds, showTime: `${hours}时${minutes}分${seconds}秒` }
}
//时间戳转换成月日时分秒
...
function isSolarSeedFn(seedId) {
  return solarTerms.reduce((pre, cur) => { return pre || Number(cur.id) === Number(seedId) }, false)
}
//作物成熟的倒计时时间
...
function calcTimeFn(land = [], packageData = []) {
  const landTime = [], landDetail = [];
  for (let i = 0; i < land.length; i++) {
    const { a: seedId, b: stage, j: seasonsNumber, s: landType, q: plantTime, o: hasFertilizer } = land[i] || {};
    const { name, harvestNum } = getSeedInfoFn(seedId) || {}
    const { ripeCountdown, ripeTime } = seedId ? getLandRipeCountdown(land[i]) : {};
    const oneLandDetail = {
      index: i + 1,
    ...
    if (stage === 0 || stage === 7 || stage === 6) {
      ...
      if (forbidLand) {
        ...
        const forbidLandRemainder = forbidLand.startTime + forbidLand.countdown - shark.getFarmTime();
        landTime.push({ time: forbidLandRemainder, tip: `[第${i + 1}块土地]==>上次禁止对该土地操作，再次操作` });
      } else {
        if (stage === 0 && !getLandSeedInputFn(i)) {
          // 当前土地如果是种植阶段且没有设置种植种子则不加入监控中
        } else if (stage === 6 && solarDelay && isSolarSeedFn(seedId) && Math.abs(ripeCountdown) < Number(solarDelay) * 60) {
          // 如果当前阶段是收获阶段且用户设置了延迟收获
          landTime.push({ time: Number(solarDelay) * 60 - Math.abs(ripeCountdown), tip: `[第${i + 1}块土地]==>${name}将延迟到${timestampToTime(ripeTime + Number(solarDelay) * 60).showTime}收获` });
        } else {
          landTime.push({ time: 2, tip: `[第${i + 1}块土地]==>${stage === 0 ? '种植' : (stage === 6 ? '收获' : "铲除")}` });
        }
      }
      oneLandDetail.ripeTime = stage === 0 ? '空地' : (stage === 7 ? '枯萎' : '成熟')
    } else {
      // 当前作物成熟的倒计时
      // 进入下个阶段需要的时间
      const nextStageTime = nextStageTimeFn(seedId, landType, plantTime)
      ...
      const fertilizer = shark.rule.fertilizer
      if (isFertilizerFn(seedId) && getPackageThingNumFn(packageData, 'tId', 3, 1).amount > 0) {
        if (fertilizer === 'last') {
          if (isFinalStageFn(seedId, landType, plantTime)) {
            // 如果当前是最后阶段
            if (!isLandFertilizerFn(land[i])) {
              landTime.push({ time: nextStageTime <= 3600 ? 2 : nextStageTime - 3600, tip: `[第${i + 1}块土地]==>施肥` });
            }
          } else {
            // 当前阶段不是最后阶段
            landTime.push({ time: ripeCountdown - 3600, tip: `[第${i + 1}块土地]==>施肥` });
          }
        } else if (fertilizer === 'every') {
          if (isFinalStageFn(seedId, landType, plantTime)) {
            if (!isLandFertilizerFn(land[i])) {
            ...
            if (!isLandFertilizerFn(land[i])) {
              ...
              //如果下个阶段是收获阶段
              if (isFinalStageFn(seedId, landType, plantTime, shark.getFarmTime() + nextStageTime)) {
                landTime.push({ time: ripeCountdown - 3600, tip: `[第${i + 1}块土地]==>施肥` });
              } else {
                // 下个阶段不是收货阶段则直接放入到倒计时数组中
                landTime.push({ time: nextStageTime, tip: `[第${i + 1}块土地]==>施肥` });
              }
            }
          }
        }
      ...
      oneLandDetail.ripeTime = secondsToHoursMinutesSecondsFn(ripeCountdown).showTime;
    }
  }
  const { time: minTime = 900, tip = '上次没有检测到可操作的土地' } = landTime.sort((a, b) => a.time - b.time)?.[0] || {};
...
// 农场收获
async function operationHarvestFn(landNum, land, packageData, landStatusUpdate) {
  let newLand = land;
...
async function listenOwnerFarmFn() {
  ...
  do {
    try {
      ...
      const { minTime, hours, minutes, seconds, tip } = calcTimeFn(userLand, packageData);
      const reloadTime = Math.min(reloadRule, minTime)
      const showTip = minTime === reloadTime ? `【农场种植】监听作物中：${hours}时${minutes}分${seconds}秒后进行下次操作${tip}` :
...
async function listenFriendFarmFn() {
  try {
    ...
    if (friendObj && friendObj instanceof Object && Object.keys(friendObj).length >= friendInfo.length) {
      ...
      for (let key in friendObj) {
        ...
        for (let i = 0; i < detail.length; i++) {
          ...
          if (stage === 0 || stage === 7 || stage === 6) {
            ...
            shark.logger.log(uin + '*' + userName + '*' + uId + '*' + i + '*' + '*' + seedId + '*' + ripeCountdown)
            shark.delay(ripeCountdown, function () {
              // 偷
              const { ecode, direction } = requestSneaking(uin, userName, uId, i, seedId)
            })
          }
        }
      }
      ...
      for (let friend of friendInfo) {
        ...
        if (!friendObj[uin]) {
          ...
          friend.detail = await requestFriendDetail(1, friend.uin, friend.userName, friend.uId);
          // 把好友信息保存到缓存中
          const cacheData = {
          ...
          shark.logger.log(`已完成${Object.keys(friendObj).length + 1}/${friendInfo.length}`)
          await shark.delay(5);
        }
...
onPanelShow = async function (refresh) {
  let tableData1 = [], columnData1 = [], tableData2 = [], columnData2 = [];
