...
pluginInputs = [
  ...
  },
  {
    ...
    placeholder: "默认15分钟",
    type: "select",
    selects: [{ name: '5分钟', value: '300' }, { name: '10分钟', value: '600' }, { name: '15分钟', value: '900' }, { name: '30分钟', value: '1800' }],
  },
];
// reqest 优化
      ...
      do {
        if (!isHope) {
          ...
          if (lv !== 40) {
            ...
            throw new Error("【微端许愿树】许愿树未达到40级暂不可许愿");
          }
          if (status === 0) {
            for (let i = 0; i < gift_list.length; i++) {
              ...
              if (id === hopeId) {
                ...
                if (ecode === 0) {
                  shark.logger.log(
                    `【微端许愿树】许愿成功:${hopeList.split("_").length * num
                    }个${hopeObj[id]}`
                  );
                  hopeFn();
                  return;
            ...
            throw new Error("【微端许愿树】此次无符合要求的许愿，将再次许愿");
          } else if (status === 4) {
...
async function requestPlanting(landNum, seedId, packageData) {
  ...
  if (Array.isArray(realSeedId)) {
    for (let i = 0; i < realSeedId.length; i++) {
      const { amount } = getPackageThingNumFn(packageData, 'cId', realSeedId[i])
      if (amount > 0) {
    ...
    const { amount } = getPackageThingNumFn(packageData, 'cId', realSeedId)
    if (amount < 1) {
...
async function requestHarvest(landNum) {
  ...
  if (code === 1) {
    shark.logger.log(`【农场种植】第${landNum + 1}块土地收获成功`)
  } else {
...
async function requestGetSeedhb() {
  ...
  if (ecode === 0) {
    ...
    shark.logger.log(`【农场种植】${direction}`);
  }
...
function forbidLandFn(landNum, time = 300) {
  ...
  if (forbidLand) {
    ...
    shark.setCache(`forbidLand${landNum}`, timeRange, timeOut)
  }
}

//通过解析接口返回值来判断当前种子是否可以施肥如果不可以则记录到cache中
function seedIsNotFertilizerFn(seedId, responseData = {}) {
  if (responseData.code === 0 && responseData.direction.indexOf('施过肥了') === '-1') {
...
// 判断的当前土地是否施肥过
function isLandFertilizerFn(b, o) {
  if (b === o) {
    return true
  } else {
    return false
...
// 施肥判断
async function plantFertilizeFn(lanNum, land, packageData) {
  const fertilizer = shark.rule.fertilizer
  const { a: seedId, b: stage, s: landType, q: plantTime, o: hasFertilizer } = land || {};
  if (seedId) {
    // 背包是有化肥
    const { amount } = getPackageThingNumFn(packageData, 'tId', 1)
    if (!isLandFertilizerFn(stage, hasFertilizer) && isFertilizerFn(seedId) && amount > 0) {
      ...
      if (fertilizer === 'last') {
        if (isFinalStageFn(seedId, landType, plantTime) && nextStageTime <= 3600) {
          //施肥
          const responseData = await requestFertilize(lanNum);
...
// 背包里物品的个数
function getPackageThingNumFn(packageData = [], idType, id) {
  const { amount = 0, isLock } = packageData.find(i => i?.[idType] === id) || {}

  return { amount, isLock }
}
// 数组去重
...
// 农场重新加载插件
function listenOwnerFarmReloadFn() {
  //重载时间
  const reload = Number(shark.rule.reload) || 900;
  shark.delay(reload, function () {
    listenOwnerFarmFn()
  })
}
// 领取节气种子
...
function calcTimeFn(land = [], packageData = []) {
  ...
  for (let i = 0; i < land.length; i++) {
    ...
    const oneLandDetail = {
      index: i + 1,
      seedName: seedId ? getSeedInfoFn(seedId).name : '空地',
      quarter: seedId ? `${seasonsNumber + 1}/${getSeedInfoFn(seedId).harvestNum}` : null,
      ripeTime: stage,
    }
    ...
    if (stage === 0 || stage === 7 || stage === 6) {
      if (shark.getCache(`forbidLand${i}`)) {
        // 暂时禁止该土地10分钟
        landTime.push({ time: shark.getCache(`forbidLand${i}`), tip: `[第${i + 1}块土地]==>上次操作失败，再次操作` });
      } else {
      ...
      // 当前作物成熟的倒计时
      const { ripeCountdown } = getLandRipeCountdown(land[i]);
      // 进入下个阶段需要的时间
      const nextStageTime = nextStageTimeFn(seedId, landType, plantTime)

      // 如果当前阶段是收获阶段，那么就倒计时1小时施肥，如果当前阶段是是成长阶段那么直接施肥
      const fertilizer = shark.rule.fertilizer
      if (isFertilizerFn(seedId) && getPackageThingNumFn(packageData, 'tId', 1).amount > 0) {
        if (fertilizer === 'last') {
          if (isFinalStageFn(seedId, landType, plantTime)) {
            // 如果当前是最后阶段
            if (!isLandFertilizerFn(stage, hasFertilizer)) {
  ...
  const { time: minTime, tip } = landTime.sort((a, b) => a.time - b.time)?.[0] || {};
  ...
  for (let i = 0; i < land.length; i++) {
    if (!shark.getCache(`forbidLand${i}`)) {
      ...
      if (stage === 0) {
        ...
        // 当果实成熟时需要判断当前种子的阶段，如果是最后一个阶段，收获了后需要铲除并种植
        await requestHarvest(landNum)
        const { harvestNum } = getSeedInfoFn(seedId)
        if (harvestNum === seasonsNumber + 1) {
          await requestScarify(landNum);
          await requestPlanting(landNum, getLandSeedInputFn(landNum), packageData)
        }
      } else if (stage === 7) {
        // 当果实枯萎时铲除并种植
...
async function listenOwnerFarmFn() {
  try {
    // 重载
    listenOwnerFarmReloadFn()
    const userLand = await requestGetUserLand();
    const packageData = await requestGetUserSeed();
    const { minTime, hours, minutes, seconds, tip } = calcTimeFn(userLand, packageData);
    if (minTime) {
      shark.logger.log(`【农场种植】监听作物中：${hours}时${minutes}分${seconds}秒后进行下次操作${tip}`)
      await shark.delay(minTime);
      shark.logger.log('【农场种植】开始操作');
      await operationFn();
      await shark.delay(2);
      await listenOwnerFarmFn();
    } else {
      shark.logger.log('【农场种植】暂无需要监听的作物')
    }
  } catch (error) {
    const message = error.toString().replace("Error: ", "");
    shark.logger.log(message);
