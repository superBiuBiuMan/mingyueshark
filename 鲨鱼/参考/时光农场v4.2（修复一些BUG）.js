...
// 定义描述
pluginDescription = "本插件不自动购买非时光种子,确保背包中其他种子数量充足。种植设置解析:比如1=#1,40|2-6=#2,2|7=6002分别表示【土地1优先种植时光种子其次种植牧草】【土地2至土地6优先种植节气种子其次种植白萝卜】【土地7仅种植夜皇后】,其中#1指代时光种子,#2指代节气种子,#3指代美食种子,40/2/6002分别为牧草/白萝卜/夜皇后的种子id,实际设置自行参考举例按格式自由搭配。查看面板可手动施肥。";
// 插件版本
pluginVersion = "20231121";
...
onPluginStart = async function() {
    ...
    let seed_flag ={
        "#3": [2, 3, 4, 5, 6, 7, 8, 9, 10, 14, 27, 37, 42, 48, 49, 50, 51, 52, 63, 65, 68, 70, 71, 77, 96, 97, 98, 99, 100, 305, 306]
    }
    
    let huafei_max =0;
    if(shark.rule.fertilizer_buy){
        if(/^\d+$/.test(shark.rule.fertilizer_buy)){
            huafei_max =Number(shark.rule.fertilizer_buy);
        }else{
            shark.logger.error("输入有误["+shark.rule.fertilizer_buy+"]");
            return;
        }
    }
            ...
            for(let i =0;i <myplant.length;i++){
                ...
                for(let j =0;j <brr.length;j++){
                    if(/^\d+$/.test(brr[j]) && !(brr[j] in myseed)){
                        ...
                        if(brr[j] in seed_flag){
                            for(let item of seed_flag[brr[j]]){
                                if(!(item in myseed)){
                                    myseed[item] ={flag: brr[j], num: 0, name: (item in seeddata?seeddata[item]["name"]:"unKnown")};
                                }
                            }
                        }
                    }
    ...
    for(;;){
        ...
        if(jieqi_has){
            ...
            if(("ecode" in solar) && solar["ecode"] ==0){
                for(let i =solar["seedList"].length -1;i >=0;i--){
                    if(solar["seedList"][i]["count"] >=4){
                        jieqi_seed.unshift(solar["seedList"][i]["id"]);
                    }else{
                        jieqi_seed.push(solar["seedList"][i]["id"]);
                    }
                    if(!(solar["seedList"][i]["id"] in myseed)){
                        myseed[solar["seedList"][i]["id"]] ={flag: "#2", num: 0, name: (solar["seedList"][i]["id"] in seeddata?seeddata[solar["seedList"][i]["id"]]["name"]:"unKnown")};
                    }
                }
                shark.logger.log("获取成功");
            }else{
        ...
        for(let item in myseed){myseed[item]["num"] =0;}
        ...
        if(fertilizer["2"]["num"] <huafei_max){
            let isbuy =shark.getCache("buyfertilizer");
            if(!isbuy){
                let buyfertilizer =await actpost("购买", "高速化肥", "{!ncf}cgi_farm_buy_tools", {type: 3, tId: 9107, number: 1});
                if(("ecode" in buyfertilizer) && buyfertilizer["ecode"] ==0){
                    shark.setCache("buyfertilizer", true, shark.getNextDayMS()-3000);
                    fertilizer["2"]["num"] +=30
                    shark.logger.log("购买成功");
                }else{
                    if("direction" in buyfertilizer){
                        if(/已|上限/.test(buyfertilizer.direction)){shark.setCache("buyfertilizer", true, shark.getNextDayMS()-3000);}
                        shark.logger.error("购买失败["+buyfertilizer.direction+"]");
                    }else{shark.logger.error("购买失败");}
                }
            }
        }
        ...
        if(("res" in timefarm) && ("farmlandstatus" in timefarm["res"])){
            let f_rule ={};
            let f_cache =shark.getCache("f_rule");
            if(f_cache !=null){
                f_rule =JSON.parse(f_cache);
            }
            ...
            for(let item of landdata){
                ...
                if(shark.rule.firstland_fer ==1 && /[37]/.test(task_spec.join("")) && !af_done && item["landid"] ==1 && item["a"] ==40 && item["b"] <6){
                    let fe =0;
                        ...
                        for(let i =0;i <20;i++){
                            if(fertilizer[fe]["num"] >0){
                                ...
                                if(("res" in actinfo) && ("farmlandstatus" in  actinfo["res"])){
                                    ...
                                    if("direction" in actinfo){
                                        if(/不[足够需能]|只能|有机|点券/.test(actinfo.direction)){break;}
                                    }
                                }
                            }
                        }
                ...
                if(shark.rule.firstland_fer ==1 && /^5$/.test(task_spec.join("")) && !af_done && item["landid"] ==1 && item["a"] ==40 && item["b"] <6){
                    isdig =await todig(item);
                    if(isdig){sleep.push(0);}
                }
                
                if(shark.rule.firstland_fer !=1 || (shark.rule.firstland_fer ==1 && item["landid"] !=1)){
                    if(/^(1|2)$/.test(shark.rule.fertilize_rule)){
                        let fe =Number(shark.rule.fertilize_rule);
                        
                        if(shark.rule.youji_fer ==1 && jieqi_seed.indexOf(item["a"]) >-1){
                            switch(fe){
                                case 1:
                                    fe =30;
                                    break;
                        ...
                        if(!isdig){
                            if(!(item["a"] in timeseed) && item["a"] !=0 && item["b"] <6){
                                let f_flag =false;
                                if(!(item["landid"] in f_rule)){
                                    f_flag =true;
                                    f_rule[item["landid"]] ={a: 0, b: 0, o: 0, q:0}
                                }else{
                                    if(f_rule[item["landid"]]["a"] !=item["a"] || f_rule[item["landid"]]["b"] !=item["b"]){
                                        f_flag =true;
                                    }
                                }
                                ...
                                if(f_flag){
                                    if(fertilizer[fe]["num"] >0){
                                        let actinfo =await tofertilize(item["landid"], fe);
                                        if(("res" in actinfo) && ("farmlandstatus" in  actinfo["res"])){
                                            f_rule[item["landid"]]["a"] =item["a"];
                                            f_rule[item["landid"]]["b"] =item["b"];
                                            sleep.push(0);
                                            item =actinfo["res"]["farmlandstatus"][0];
                                        }else{
                                            if("direction" in actinfo){
                                                if(/不[足够需能]|只能|有机|点券/.test(actinfo.direction)){
                                                    f_rule[item["landid"]]["a"] =item["a"];
                                                    f_rule[item["landid"]]["b"] =item["b"];
                                                }
                                            }
            ...
            shark.setCache("f_rule", JSON.stringify(f_rule), 4070880000000);
        }
        ...
        for(let i =1;i <=sleep[0];i++){
            ...
            let next_t =new Date(shark.getNextDayMS() + 6000);
    ...
    shark.logger.error("农场出现故障,暂停30秒后重试");
...
async function tofertilize(lid, fl){
    let check ={};
    for(let i =0;i <3;i++){
        check =await actpost("施肥", "-土地"+lid+"["+fertilizer[fl]["name"]+"]", "{!nc}cgi_farm_time_space", {act: "fertilize", landid: lid, toolid: fl});
        if(("ecode" in check) && check["ecode"] ==0){
            ...
            if("direction" in check){
                ...
                if(/不[足够需能]|只能|有机|点券/.test(check.direction)){break;}
        ...
        if(crr.length >0){
            for(let k =0;k <crr.length;k++){
                if(/^\d+$/.test(crr[k])){
                    ...
                    if(crr[k] =="#2"){
                        for(let f =0;f <jieqi_seed.length;f++){
                            if((jieqi_seed[f] in myseed) && myseed[jieqi_seed[f]]["num"] >0){
                                plantcid =jieqi_seed[f];
                                break;
                            }
                        }
                    }else{
        ...
        for(let i =0;i <3;i++){
            ...
            if(("ecode" in check) && check["ecode"] ==0){
                ...
                if("direction" in check){
                    ...
                    if(/不[足够]/.test(check.direction)){break;}
...
async function gettimefarm(){
    let index ={}
    for(let i =0;i <3;i++){
        index =await actpost("获取", "时光农场信息", "{!nc}cgi_farm_time_space", {act: "index"});
